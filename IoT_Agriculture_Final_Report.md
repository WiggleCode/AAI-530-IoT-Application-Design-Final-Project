...
### Class Imbalance in Actuator States

Analysis of actuator activation frequencies revealed significant class imbalance:

- **Fan**: 31.0% ON, 69.0% OFF
- **Watering Pump**: 23.8% ON, 76.2% OFF
- **Water Pump**: 22.6% ON, 77.4% OFF

For classification tasks predicting actuator states, this imbalance presents challenges. A naive classifier predicting "always OFF" would achieve ~70-77% accuracy without learning meaningful patterns. We addressed this through multiple strategies in our modeling approach:

1. **Evaluation Metrics**: Used precision, recall, and F1-scores rather than accuracy alone
2. **Stratified Splitting**: Ensured train/test splits maintained class proportions
3. **Class Weights**: Planned implementation of class weights in classifiers to penalize majority class errors

### Humidity by Actuator State Analysis

Box plot analysis comparing humidity distributions when actuators are ON versus OFF revealed operational impacts:

- **Fan ON**: Mean humidity decreased by ~8 percentage points compared to Fan OFF, confirming the fan's effectiveness in reducing moisture
- **Watering Pump ON**: Mean humidity increased by ~5 percentage points, showing the moisture impact of irrigation
- **Water Pump ON**: Showed minimal direct impact on humidity, suggesting its primary role is reservoir filling rather than direct environmental control

These findings validate the physical relationships in the greenhouse system and demonstrate that actuator states contain predictive information for environmental conditions.
...
**Linear Regression**: A simple linear model was fitted to predict water level from temperature, humidity, and soil nutrients (N, P, K). Features were standardized using StandardScaler. The model achieved:
...
Building on the strong Random Forest results, we implemented an XGBoost regressor as our primary predictive model for humidity estimation.
...
To satisfy both the deep learning requirement and demonstrate time-series capabilities distinct from the XGBoost cross-sensor regression, we implemented an LSTM model for water level prediction.

#### Distinction from XGBoost Approach

While the XGBoost model performs **cross-sensor regression** (predicting one sensor from current values of others), the LSTM performs **time-series forecasting** (predicting future values from historical sequences). These represent fundamentally different tasks:

- **XGBoost**: "Given current temperature, nutrients, and actuator states, what should humidity be right now?" - Useful for sensor validation and fault detection
- **LSTM**: "Given the past sequence of water level readings, what will water level be in the next time step?" - Useful for anticipating future conditions and proactive control

This distinction ensures the LSTM serves both as our deep learning model and our time-series model without redundancy.
...
#### Table 3
*Model Comparison Summary by Use Case*

| Use Case | Best Model | Metric | Performance | Deployment Target | Justification |
|----------|-----------|--------|-------------|-------------------|---------------|
| Real-time humidity prediction | XGBoost | R² | 0.9006 | Edge (Raspberry Pi) | Best accuracy with <0.1ms inference |
| Sensor fault detection | XGBoost | MAE | 4.06% | Edge | Fast, interpretable deviations |
| Actuator control | RF Classifier (balanced) | F1-Score | 0.800 | Edge | Balanced precision/recall for control |
| Water level forecasting | LSTM | MAE | 6.16% | Cloud | Captures temporal patterns |
| Long-term trend analysis | LSTM | - | - | Cloud | Sequential dependencies |
| Resource planning | Random Forest Reg. | RMSE | 4.22 | Cloud | Good accuracy, low complexity |

*Note*. RF = Random Forest. Edge deployment refers to on-device processing (Raspberry Pi 4, 4GB RAM). Cloud deployment leverages remote GPU resources.

**Key Findings from Tables 1-3**:

1. **Tree-based methods outperform linear models**: Random Forest achieved 39% lower MSE (17.81 vs 29.16) than Linear Regression for water level prediction, demonstrating the value of capturing non-linear relationships and feature interactions in greenhouse data.

2. **XGBoost provides optimal regression performance**: With R² of 0.9006 and RMSE of 7.15% for humidity prediction (Table 1), XGBoost delivers excellent accuracy while maintaining fast inference speeds (<0.1ms) suitable for edge deployment.

3. **Class balancing significantly improves classification**: Table 2 shows that Random Forest with balanced class weights improved F1-score from 0.748 to 0.800 for water pump prediction, with balanced precision (0.812) and recall (0.789). This 7% improvement in F1-score validates our class imbalance mitigation strategy.

4. **LSTM adds time-series capabilities**: While computationally more expensive (42.3s training, 15ms inference), the LSTM model captures temporal dependencies unavailable to cross-sensor regression approaches. The 6.16% MAE for water level prediction demonstrates competent performance, with room for improvement through multi-timestep sequences.

5. **Baseline models validate approach**: The strong performance of simpler models (Random Forest: R² = 0.8654) confirms that the dataset contains learnable patterns, while the incremental improvement from XGBoost (R² = 0.9006) justifies the added complexity for production deployment.

6. **Inference speed enables edge deployment**: Table 1 shows XGBoost achieves <0.1ms inference time, making it 150x faster than LSTM (15ms). This speed advantage is critical for real-time actuator control on resource-constrained edge devices.
...
|------|------------------|---------------------|---------------|
| Humidity spike detection | <1 second | Edge | Plant stress occurs quickly |
| Actuator activation | <1 second | Edge | Immediate response prevents damage |
| Sensor validation | <5 seconds | Edge | Detect and flag faulty sensors |
| Hourly trend analysis | Minutes | Cloud | Planning, not reactive |
| Daily forecasting | Hours | Cloud | Strategic scheduling |
| Model retraining | Days | Cloud | Batch process with accumulated data |
...
1. **Initial Setup**:
   - Deploy pre-trained XGBoost model to edge devices
   - Configure cloud infrastructure (database, API, dashboard)
   - Establish MQTT broker for edge-cloud communication

2. **Operational Loop**:
   - Edge device collects sensor readings every 5 minutes
   - XGBoost predicts expected humidity from other sensors
   - If prediction deviates from actual (>10% difference), flag potential sensor fault
   - Make actuator decisions based on sensor + predictions
   - Batch upload data to cloud every 5 minutes

3. **Cloud Analytics**:
   - Store sensor data and predictions in time-series database
   - Run LSTM forecasting models every 6 hours for next-day predictions
   - Monitor model drift (compare predictions to actuals over rolling window)
   - Trigger model retraining if drift exceeds threshold

4. **Model Updates**:
   - Train improved models on cloud using accumulated historical data
   - Validate new models on held-out test set
   - Deploy to staging edge device for A/B testing
   - Roll out to production edge devices if performance improves
...
### Addressing Model Requirements

Per the project requirements, we have clearly distinguished:

- **Baseline Traditional ML**: Linear Regression and Random Forest establish performance benchmarks
- **Advanced Traditional ML**: XGBoost provides production-ready cross-sensor regression
- **Deep Learning**: LSTM neural network fulfills deep learning requirement
- **Time-Series Analysis**: LSTM's sequential processing fulfills time-series requirement with tasks distinct from XGBoost's cross-sensor regression

The LSTM serves both deep learning and time-series requirements through fundamentally different prediction tasks: XGBoost answers "what should this sensor read now?" while LSTM answers "what will this sensor read next?"
...
## Appendix: Model Hyperparameters and Configuration
...
- Missing Value Handling: Dropped 2 rows with missing dates
- Feature Engineering: Extracted hour, day, month from datetime
- Actuator Encoding: Kept only ON columns, dropped redundant OFF columns

---

*Word Count: ~5,500 words | Pages: 5 (formatted)*
